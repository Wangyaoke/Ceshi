...
public class ChartView extends View {
    ...
    //x轴各个坐标点水平间距
    private int interval = dpToPx(150);
    //背景颜色
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        Object chartViewJudge = SharedPreferenceUtils.get(getContext(), "ChartViewJudge", "");
        if (changed) {
...
//            //X轴文本最大高度
            ...
            for (int i = 0; i < xValue.size(); i++) {//求取x轴文本最大的高度
                ...
                int w = rect.width();
                int h = rect.height();
                if(chartViewJudge.equals("EnterpriseBigData")) {
                    if (w > textXHeight)
                        textXHeight = w;
                }else if(chartViewJudge.equals("PurchaseBigData")){
                    if (h > textXHeight)
                        textXHeight = h+85;
                }
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (i == selectIndex - 1) {
                linePaint.setStyle(Paint.Style.FILL);
                linePaint.setColor(Color.parseColor("#4A9FF0"));
                canvas.drawCircle(x, y, dp5, linePaint);
                linePaint.setColor(Color.parseColor("#97C2EC"));
                canvas.drawCircle(x, y, dp5, linePaint);
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < yValue.size(); i++) {
            if(0==i) {
                ...
                mPaint.setColor(getResources().getColor(R.color.ccc));
                Path mPath = new Path();
                DashPathEffect effects = new DashPathEffect(new float[]{0,0}, 0);
                mPaint.setPathEffect(effects);
                canvas.drawPath(mPath, mPaint);
                canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2, xInit + interval * (xValue.size() - 1) + (width - xOri) * 0.1f, yOri - yLength * i + xylinewidth / 2, xyPaint);
                oneLineY = yOri - yLength * i + xylinewidth / 2;
                oneLineX = xOri;
                xyPaint.setColor(linecolor);
            }else{
                //初始化，并打开抗锯齿
                ...
                Path mPath = new Path();
                mPath.moveTo(xOri, yOri - yLength * i + xylinewidth / 2);
                mPath.lineTo(xInit + interval * (xValue.size() - 1) + (width - xOri) * 0.1f, yOri - yLength * i + xylinewidth / 2);
                //数组含义：里面最少要有2个值，值的个数必须是偶数个。偶数位（包含0），表示实线长度，奇数位表示断开的长度
                DashPathEffect effects = new DashPathEffect(new float[]{20,4}, 0);
                mPaint.setPathEffect(effects);
                canvas.drawPath(mPath, mPaint);
            }
        ...
        //绘制X轴坐标
        canvas.drawLine(xOri, yOri + xylinewidth / 2, width+120, yOri + xylinewidth / 2, xyPaint);
        //绘制x轴箭头
        ...
        //画阴影  根据xy轴以及每个点的位置，画出一个不规则图形进行填充
        //移动到x轴的位置
        Path path1 = new Path();
        path1.moveTo(xOri, yOri + xylinewidth / 2);
        Paint shadePaint = new Paint();
        Shader mShader = new LinearGradient(xOri, yOri + xylinewidth / 2,0,0,
                new int[]{Color.parseColor("#FAFCFE"),Color.parseColor("#F5F9FE"),Color.parseColor("#EEF5FE"), Color.parseColor("#E7F2FD"),Color.parseColor("#E4F0FD"),Color.parseColor("#DDECFD")}, null, Shader.TileMode.CLAMP);
        ...
        path1.lineTo(xLength - dpToPx(12), yOri + xylinewidth / 2);
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >=xOri) {//只绘制从原点开始的区域
                ...
                if(chartViewJudge.equals("EnterpriseBigData")) {
                    //字体改变角度
                    canvas.save();
                    canvas.rotate(45, x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + 50);
                    textTop = dpToPx(2);
                }else if(chartViewJudge.equals("PurchaseBigData")){
                    textTop = dpToPx(10);
                }
                xyTextPaint.setColor(Color.GRAY);
                xyTextPaint.setStyle(Paint.Style.FILL);
                xyTextPaint.getTextBounds(text, 0, text.length(), rect);
                if (i == selectIndex - 1) {
                    xyTextPaint.setColor(linecolor);
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + textTop + rect.height(), xyTextPaint);
                } else {
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + textTop + rect.height(), xyTextPaint);
                }
                if(chartViewJudge.equals("EnterpriseBigData")) {
    ...
    private void clickAction(MotionEvent event) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (eventX >= x - dp8 && eventX <= x + dp8 &&
                ...
                selectIndex = i + 1;
                chartViewListnear.ChartViewClick(selectIndex);
                invalidate();
                return;
            }
            //X轴刻度
    ...
    public  interface ChartViewListnear{
        void ChartViewClick(int Index);
    }
    private ChartViewListnear chartViewListnear;
    public void setChartViewListnear(ChartViewListnear chartViewListnear){
        this.chartViewListnear = chartViewListnear;
    }
    /**
     * 获取速度跟踪器
     *
